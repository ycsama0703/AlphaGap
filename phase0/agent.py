...
_TOOLDOC = {
    "get_fundamentals": "get_fundamentals(symbol) -> recent quarterly income-statement rows (revenue, net_income, eps, ...)",
    "get_earnings_history": "get_earnings_history(symbol) -> recent rows (actual_eps, estimated_eps, surprise_pct, actual_revenue, report_date)",
    "get_news": "get_news(symbol, query) -> recent headlines+summaries (optionally filtered by query)",
    "search_transcript": "search_transcript(symbol, year, quarter, query) -> earnings-call snippets matching query",
}

# Agent CONFIGS = different "systems under test" — they differ ONLY by which tools they can use, so the
# benchmark can isolate how evidence access changes evidence sufficiency (not just final accuracy).
CONFIGS = {
    "full": list(_TOOLDOC),                                  # all tools incl. document retrieval
    "numeric_only": ["get_fundamentals", "get_earnings_history"],   # data but NO documents → can't ground driver claims
    "no_tools": [],                                          # parametric memory only
}

_SYS_HEAD = """你是一个金融研究 agent。{toolclause}然后把回答拆成原子 claim,并标注每个 claim 依据哪条证据。
...
"""
_SYS_RULES = """规则:数值类 claim 必须来自 get_fundamentals/get_earnings_history(若可用)。营收同比(revenue_yoy_pct)=get_fundamentals 里 ≤as_of 的最近一季 revenue 对比约四个季度前同季 revenue,(新-旧)/旧×100。定性类(原因/驱动/管理层说法)必须引 transcript/news 的具体内容。
不要编造证据;若工具查不到支撑(或无工具可用),claim 的 evidence_ref 写 "无直接证据"。最多 8 步,尽快给 final。"""


def _build_system(allowed: list[str]) -> str:
    if allowed:
        clause = "用给的工具回答问题,"
        tail = "可用工具:\n" + "\n".join(_TOOLDOC[t] for t in allowed) + "\n"
    else:
        clause = "你没有任何工具,只能凭自身知识直接回答问题,"
        tail = ""
    return _SYS_HEAD.format(toolclause=clause) + tail + _SYS_RULES


def run_agent(task: dict, *, client, allowed_tools: list[str] | None = None) -> dict:
    """Run the ReAct loop for one task. allowed_tools = which tools this agent config may use (None=all).
    Returns {answer, numeric_answers, claims, trajectory, n_tool_calls}."""
    c = fa.load_client()
    allowed = list(_TOOLDOC) if allowed_tools is None else allowed_tools
    tools = {t: TOOLS[t] for t in allowed}
    system = _build_system(allowed)
    no_tools = not allowed
    ...
    for step in range(MAX_STEPS):
        try:
            out = client.chat_json(system=system, user=msg, temperature=0.0, max_tokens=1400)
        except Exception as e:
            traj.append({"error": f"llm: {str(e)[:120]}"})
            break
        if "final" in out:
            if tools and n_calls == 0 and not nudged:   # force ≥1 retrieval (skip if config has no tools)
                nudged = True
        ...
        if tool not in tools:
            msg += f"\n[观察] 工具 {tool} 不可用;本配置可用: {list(tools) or '无(请直接 final)'}"
